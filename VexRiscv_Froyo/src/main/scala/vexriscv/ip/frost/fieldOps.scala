...
case class fsum_mod() extends Component{
  ...
  val io = new Bundle {
    ...
    val OUT = out Vec(Bits(32 bits), 8)
    val OUTadd = out Vec(Bits(32 bits), 8)
    val carryOut = out Bits (1 bits)
  }

//  println("here F")
  val Med = Vec.fill(8)(Bits(33 bits))
  Med(0) := (io.A(0).asUInt.resize(33 bits) + io.B(0).asUInt.resize(33 bits) + io.carryIn.resize(33 bits).asUInt).asBits
  ...
  for(i <- 0 until 8){
    io.OUTadd(i) := Med(i)(31 downto 0)
  }
  io.carryOut := Med(7)(32 downto 32)
//  println("here G")
...
case class fsum_mod_wrapper() extends Component{
  ...
  val coreArea = new ClockingArea(clkDomain) {
    ...
    carry_reg := RegNextWhen(io.carryIn, io.start, B(0, 1 bits))

    add_comb.io.carryIn := carry_reg
    for (i <- 0 until 8) {
      add_comb.io.A(i) := A_reg(32 * (i + 1) - 1 downto 32 * i)
      add_comb.io.B(i) := B_reg(32 * (i + 1) - 1 downto 32 * i)
      io.OUT(32 * (i + 1) - 1 downto 32 * i) := RegNextWhen((io.modeMod === False)? add_comb.io.OUTadd(i) | add_comb.io.OUT(i), RegNext(io.start), B(0, 32 bits))
    }
    io.carryOut := RegNextWhen((io.modeMod === False)? add_comb.io.carryOut | B(0, 1 bit), RegNext(io.start), B(0, 1 bits))
...
case class fsub_mod()extends Component{
  ...
  val io = new Bundle {
    ...
    val carryIn = in Bits (1 bits)
    val OUT = out Vec(Bits(32 bits), 8)
    val OUTsub = out Vec(Bits(32 bits), 8)
    val carryOut = out Bits (1 bits)
  }

  val Med = Vec.fill(8)(Bits(33 bits))
  Med(0) := (io.A(0).resize(33 bits).asSInt - io.B(0).resize(33 bits).asSInt - io.carryIn.resize(33 bits).asSInt).asBits
...
case class fsub_mod_wrapper() extends Component{
  ...
  val coreArea = new ClockingArea(clkDomain) {
    ...
    sub_comb.io.carryIn := carry_reg
    for (i <- 0 until 8) {
      sub_comb.io.A(i) := A_reg(32 * (i + 1) - 1 downto 32 * i)
      sub_comb.io.B(i) := B_reg(32 * (i + 1) - 1 downto 32 * i)
      io.OUT(32 * (i + 1) - 1 downto 32 * i) := RegNextWhen((io.modeMod === False)? sub_comb.io.OUTsub(i) | sub_comb.io.OUT(i), RegNext(io.start), B(0, 32 bits))
    }
    io.carryOut := RegNextWhen((io.modeMod === False)? sub_comb.io.carryOut | B(0, 1 bit), RegNext(io.start), B(0, 1 bits))
    io.done := RegNext(RegNext(io.start))
  }
...
case class fmac_limb1() extends Component {
  val io = new Bundle{
    val A = in Bits (22 bits)
    val B = in Bits (16 bits)
...
case class fmac_limb() extends Component {
  ...
  val mul_OUT = Bits(38 bits)
  mul_OUT := (io.A.asUInt * io.B.asUInt).asBits

  io.mulOut := mul_OUT

  val add_OUT = Bits(38 bits)
  add_OUT := (mul_OUT.asUInt + io.Carry.asUInt)(37 downto 0).asBits
...
//  val fmul_inst = fmul_mod_combined_new_timing()
  ...
  fsum_inst.io.carryIn := B(0, 1 bits)
  ...
  val coreArea = new ClockingArea(clkDomain) {
    ...
    io.done := (io.mode_mod19 === B(1, 1 bits))? RegNext(fmul_inst.io.cnt_out === U(20, 5 bits)) | RegNext(fmul_inst.io.cnt_out === (io.size_A.asUInt.resize(5 bits) + U(2, 5 bits)))
