...
class IBusCachedPlugin(resetVector : BigInt = 0x80000000l,
  ...
  var iBusDebugPc : UInt = null
  var iBusDebugIsValid : Bool = null
  var iBusDebugIsStuck : Bool = null
  var iBusDebugData : Bits = null
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    pipeline plug new FetchArea(pipeline) {
      ...
      iBusDebugIsValid = Bool()
      iBusDebugIsStuck = Bool()
      iBusDebugPc = UInt(32 bits)
      iBusDebugData = Bits(32 bits)

      iBusDebugIsValid := cache.io.cpu.decode.isValid
      iBusDebugIsStuck := cache.io.cpu.decode.isStuck
      iBusDebugPc := cache.io.cpu.decode.pc
      iBusDebugData := cache.io.cpu.decode.data
