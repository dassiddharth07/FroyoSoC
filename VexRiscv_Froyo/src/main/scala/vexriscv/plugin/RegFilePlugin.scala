...
class RegFilePlugin(regFileReadyKind : RegFileReadKind,
  ...
  var dbgReg_readAddress1 : UInt = null
  var dbgReg_readAddress2 : UInt = null
  var dbgReg_readRs1Data : Bits = null
  var dbgReg_readRs2Data : Bits = null

  var dbgReg_writeValid : Bool = null
  var dbgReg_writeAddress : UInt = null
  var dbgReg_writeData: Bits = null
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    // Debug
    dbgReg_readAddress1 = UInt(5 bits)
    dbgReg_readAddress2 = UInt(5 bits)
    dbgReg_readRs1Data = Bits(32 bits)
    dbgReg_readRs2Data = Bits(32 bits)

    dbgReg_writeValid = Bool()
    dbgReg_writeAddress = UInt(5 bits)
    dbgReg_writeData = Bits(32 bits)
    ...
    readStage plug new Area{
      ...
      dbgReg_readAddress1 := regFileReadAddress1
      dbgReg_readAddress2 := regFileReadAddress2
      dbgReg_readRs1Data := rs1Data
      dbgReg_readRs2Data := rs2Data
    ...
    writeStage plug new Area {
      ...
      dbgReg_writeValid := regFileWrite.valid
      dbgReg_writeAddress := regFileWrite.address
      dbgReg_writeData := regFileWrite.data
