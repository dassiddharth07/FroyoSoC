...
// CFU plugin supports only custom1 as of now
#define STR1(x) #x
#define STR(x) STR1(x)
#define CUSTOMX_OPCODE(x) CUSTOM_ ## x
#define CUSTOM_0 0b0001011

#define CUSTOMX(X, funct3, rd, rs1, rs2, funct7) \
  CUSTOMX_OPCODE(X)                     |             \
  (rd                 << (7))           |             \
  (funct3             << (7+5))         |             \
  (rs1                << (7+5+3))       |             \
...
#define CFU_INST(rd, rs1, rs2, funct3, funct7) \
	ROCC_INSTRUCTION_R_R_R(0, rd, rs1, rs2, funct3, funct7, 10, 11, 12)

#define ROCC_INSTRUCTION_R_R_R(X, rd, rs1, rs2, funct3, funct7, rd_n, rs1_n, rs2_n) { \
    ...
    asm volatile (                                                           \
        ".word " STR(CUSTOMX(X, funct3, rd_n, rs1_n, rs2_n, funct7)) "\n\t"  \
        : "=r" (rd_)                                                         \
